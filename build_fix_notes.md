...
## Workflow hardening inspection (2026-08-14, from user checklist paste)
Facts from repo: package.json scripts exist: check=tsc --noEmit, test=vitest run (valid). packageManager: pnpm@9.12.0, pnpm-lock.yaml present (443KB). No engines field. version: 1.0.0 in package.json AND app.config.ts version "1.0.0". app.json has owner TO_BE_REPLACED, slug iso-tile-map, sdkVersion 54.0.0. NO expo.eas.projectId anywhere (not linked). eas.json: cli appVersionSource remote, preview apk buildType with :app:assembleRelease + autoIncrement, production AAB. Workflow file: .github/workflows/release-apk.yml (only workflow; no "debug workflow" exists — user's note about debug workflow is preemptive guidance). Orientation landscape in app.config.ts.
Planned rewrite: (1) fail-fast EXPO_TOKEN check in workflow; (2) use EAS CLI for build start + ID parse robustly (strip "Builds:" text, jq parse first object); (3) track via `eas build:list`? No — polling via Expo API with account/slug from app.json + fallback to app.config.ts slug; validate response format (resp.data.status, resp.data.artifacts.buildUrl) with explicit error messages; (4) APK download with non-empty validation (file size > 0); (5) release tag collision safety: tag = v$(date %Y%m%d%H%M%S) on dispatch OR use input version, fail cleanly if exists → use softprops overwrite false (default) so duplicate fails loudly; better: auto-generate timestamp tag only on dispatch without input; (6) remove `|| true` from version increment — keep the step but no silent swallow; make version bump fail with clear message if eas.projectId missing AND user wants remote increment (warn clearly in log); (7) keep triggers: tags v* + workflow_dispatch with bump_type input; (8) note to user: EXPO_TOKEN secret required + eas build:configure needed to set projectId; owner placeholder must be replaced.
